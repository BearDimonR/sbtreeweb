...
const getOp = (key) => {
  switch (key) {
    case "start":
      return ({ start, end }, b) =>
        moment(b).isBefore(end) && moment(b).isAfter(start);
    case "name":
...
export const getEventNames = async () => {
  ...
  return _.sortBy(_.uniq(events.map((e) => e.name.trim())), (a) =>
    a.toLowerCase()
  );
};
export const getEventСategories = async () => {
  ...
  return _.sortBy(_.uniq(events.map((e) => e.category.trim())), (a) =>
    a.toLowerCase()
  );
};
...
export const getEventPeople = async (id) => {
  ...
  const res = people.reduce((res, obj) => {
    ...
    return [
      ...res,
      {
        ..._.omit(obj, ["email", "about", "telephone", "start", "end"]),
        role: epValue.role,
        about: epValue.about,
...
export const getActivity = async (id, additional = true) => {
  ...
  if (additional) {
    return {
      ...activity,
      person: await getPerson(activity.person_id, false),
      event: await getEvent(activity.event_id, false),
    };
  }
  return activity;
};
...
export const putActivity = async (data) => {
  ...
  //TODO errors?
  activity = _.merge(activity, _.omitBy(data, _.isEmpty), {
    person_id: person.id,
    event_id: event.id,
  });
  return activity;
