...
const model = Schema.Model({
  ...
  about: StringType().isRequired(reqField).minLength(30, 'Мінімум 30 символів').maxLength(3000, 'Максимум 3000 символів'),
  status: StringType().isRequired(reqField),
  email: StringType().isRequired(reqField).isEmail('Не вірний формат (example@example.com)'),
  telephone: StringType().isRequired(reqField).pattern(/^[+][0-9]{2}?[\s]?[(]?[0-9]{3}[)]?[\s]?[0-9]{3}[-\s]?[0-9]{4}$/im, 'Не вірний формат (+38(098) 123-1234)'),
});
...
const getInitial = (person) => ({
        ...
        dateRangePicker: [Date.parse(person.start) || new Date(), Date.parse(person.end) || new Date()],
        about: person.about || '',
...
const getInput = (props) => <Input as="textarea" {...props} style={{height: '80px'}} />;
