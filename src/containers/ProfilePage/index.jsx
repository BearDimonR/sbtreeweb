...
const PersonPage = ({
    user, activity, fullNames, eventNames, loadPerson, loadActivity, setActivity, loadNames, loadFullNames, editPerson, editActivity, removePerson, removeActivity, setContentIsLoading
}) => {
    const {id} = useParams();
    ...
    const handleEdit = useCallback(() => {
        wrapInSetContentLoading(() => loadPerson(user.id), () => setEditing(true));
    }, [wrapInSetContentLoading, setEditing, loadPerson, user]);

    const handleDelete = useCallback(() => {
        removePerson(user.id);
        wrapInSetContentLoading(() => removePerson(user.id), () => {
    ...
    return <div className={style.profileContainer}>
        <ProfileView user={user} onEdit={handleEdit} onDelete={handleDelete} onActivityEdit={handleActivityEdit} onActivityDelete={handleActivityDelete} />
        <PersonModal open={editing} onClose={handleModalClose} user={user} person={user} onSubmit={handleSubmit} />
