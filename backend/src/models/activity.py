...
class Activity(BaseEntity):
    ...
    __table_args__ = (
        db.UniqueConstraint("person_id", "event_id", name="_event_location_uc_"),
    )

    event = relationship("Event", back_populates="activities")
    person = relationship("Person", back_populates="activities")

    types = {**BaseEntity.types, "person_id": "id", "event_id": "id"}
    ...
    def filter_data(cls, dataframe):
        ...
        return dataframe[
            (dataframe["person_id"].notna()) & (dataframe["event_id"].notna())
        ]
    ...
    def to_dict_with_event(self):
        return {**self.to_dict(), "event": self.event.to_dict()}

    def to_dict_with_person(self):
        return {**self.to_dict(), "person": self.person.to_short_dict()}
