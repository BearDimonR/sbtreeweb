...
class BaseService:
    ...
    def get_all(self, sort=None, params=None, search=None, page=None, filters=None):
        ...
        if params:
            query = query.with_entities(*[column(self.model.get_key(param)) for param in params]).select_from(
                self.model).distinct()
        if filters:
            filter_obj = []
            for col in filters:
        ...
        if search:
            query = query.filter(*[column(self.model.get_key(s[0])).startswith(s[1]) for s in search])
        if sort:
    ...
    def get_all_param(self, params):
        return list(
            map(lambda x: {param: self.model.transform_field(param, x[i]) for i, param in enumerate(params)},
